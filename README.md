...
| **Helpers & weekly feed report** | Tag each animal with the helper/breeder who feeds it, then from that helper's page generate a **Weekly feed report** — every one of their animals' **latest weight + current ration** (amounts shown as clean fractions, AM/PM meals labeled), pre-filled and **editable** (add "slop", a new-supplement note, etc.) so you can **Copy or Share it straight into a text message** alongside your weekly videos. |
