...
| **Record Book & readiness** | A compiled career achievement record — team-wide or per animal: class wins, banners & champions, shows, placings, premiums, sale totals and points, grouped by show. **Printable / Save-as-PDF** for a physical record book or sale packet. A **Record Book Readiness** score (e.g. *93% — missing: final reflection*) checks the 14 things a complete youth-project record needs — identification, acquisition, purchase price, weigh-ins, feed, expenses, income/sale, health, photos, show result, plus **project goals, hours, learning journal, skills and a final reflection** — auto-detecting what already exists and linking straight to whatever's missing. The all-animals view ranks every project by how complete it is, so you know exactly what to finish before the book is due. |
| **Coach Scorecard** | On each animal's **Progress → Scorecard**, score the traits that matter — **muscle · width · shape · finish · freshness · skin & hair · mobility · showmanship · brace · presentation** (fully **customizable** — add leg-set, hip, base width, or drop what you don't use) — on a 1–10 scale with notes. Every evaluation is dated and attributed to a person; the app never auto-grades. It then **graphs the overall trend** and shows each trait's latest score with a bar and its **change since the first look** (e.g. *Muscle 8 · ▲ +2*), so a coach can see real improvement over a season, not just vibes. |
...
| **Season Review** | End-of-project **"what worked?"** analytics per animal, all from records you already kept: total gain, **cost of the project** (feed + bedding + expenses), **cost per pound of gain**, income and **net**, every **feed program with the gain that happened during it** (best-gain flagged), **weight at each show**, your **best result and the program active when you earned it**, the **coach adjustments you accepted**, and a **"what to remember next year"** reflection. Copy the whole summary to a text. A **barn rollup** ranks every animal by gain, cost-per-pound and net across seasons — institutional memory for the whole operation. Careful wording throughout: it reports what happened *during* a program, never that a product *caused* it. |
...
- **Public marketing site** — `index.html` serves a crawlable marketing homepage to new visitors and search engines, while **existing users, installed (standalone) apps, and deep links boot straight into the app** (a tiny boot script decides, so the app itself is unchanged). `features/*.html` and `for-show-*.html` are statically-rendered SEO pages generated by `scripts/gen-marketing.mjs`; `sitemap.xml` + `robots.txt` are generated alongside. No private data is ever exposed on these pages.
